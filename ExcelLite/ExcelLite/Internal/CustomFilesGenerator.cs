...
namespace ExcelLite.Internal
{
    public class CustomFilesGenerator
    {
        ...
        public async Task GenerateSheet(Sheet sheet, StreamWriter streamWriter, CancellationToken ct = default)
        {
            ...
            else if (sheet.AsyncData is not null)
            {
                ...
                {
                    throw new ArgumentException("AsyncData should be type of IAsyncEnumerable<XYZ>");
                }

                recordsType = asyncEnumerableType.GenericTypeArguments[0];
            }
            else
            {
                throw new ArgumentException("Cannot set Data and AsyncData!");
            }
            ...
            // TODO!
            if (sheet.Data is not null && sheet.AsyncData is null)
            {
            ...
            else if (sheet.Data is null && sheet.AsyncData is not null)
            {
        ...
        private void WriteRow(StreamWriter streamWriter, List<Header> headers, ref int rowIndex, object data, Sheet sheet)
        {
            ...
            foreach (var header in headers)
            {
                ...
                if (value is not null || sheet.UseBorders)
                {
                    //<c r="A1"><v>11</v></c>
                    streamWriter.Write("<c r=\"");
                    GetAndWriteCellId(streamWriter, rowIndex, columnIndex);
                    streamWriter.Write("\" ");
                    ...
                    else if (value is ICustomCellExporter customCellExporter)
                    {
                        streamWriter.Write("t=\"inlineStr\"><is><t>");
                        streamWriter.Write(customCellExporter.GetValue());
                        streamWriter.Write("</t></is>");
                    }
                    else
                    {
        ...
        public void GetAndWriteCellId(StreamWriter streamWriter, int rowIndex, int columnIndex)
        {
